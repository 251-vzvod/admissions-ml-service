...
TRAINING_JSONL = EXPORTS_DIR / "training_dataset_v1.jsonl"
TRAINING_CSV = EXPORTS_DIR / "training_dataset_v1.csv"
MANIFEST_JSON = EXPORTS_DIR / "training_dataset_v1_manifest.json"
...
def load_candidate_pool() -> dict[str, dict[str, Any]]:
    ...
    for path in [SEED_JSONL, SYNTHETIC_V1_JSONL, CONTRASTIVE_V2_JSONL, TRANSLATED_V3_JSONL]:
...
def build_split_map(candidate_ids: list[str]) -> dict[str, str]:
    ...
    for source_group, ids in grouped.items():
        ...
        if test_count <= 0:
            raise ValueError(f"Split allocation failed for {source_group}")
    return split_map
...
def build_rows() -> list[dict[str, Any]]:
    ...
    if set(candidate_ids) != set(pool):
        ...
        raise ValueError(
            f"Candidate pool mismatch. Missing in pool: {missing_in_pool[:5]} Extra in pool: {extra_in_pool[:5]}"
        )
...
def write_manifest(rows: list[dict[str, Any]]) -> None:
    ...
    manifest = {
        ...
        "generated_from": {
            ...
            "seed_payloads": str(SEED_JSONL),
            "synthetic_v1_payloads": str(SYNTHETIC_V1_JSONL),
            "contrastive_v2_payloads": str(CONTRASTIVE_V2_JSONL),
            "translated_v3_payloads": str(TRANSLATED_V3_JSONL),
        },
        "row_count": len(rows),
        ...
        "notes": [
            "Candidate-level canonical training table for first offline ranker experiments.",
